...
import { Key, RefreshCw, Shield, Search, Copy, Check, ExternalLink } from 'lucide-react'
...
export default function ProxyKeysPage() {
  ...
  const { data, isLoading } = useOrderItems({ page, limit: 100, type: type || undefined, status: status || undefined, search: search || undefined }, false)
    ...
    <Box sx={{ p: 3, maxWidth: 1200, mx: 'auto' }}>
      ...
      <Grid2 container spacing={1.5} sx={{ mb: 2 }}>
        <Grid2 size={{ xs: 6, sm: 3 }}>
          <CustomTextField fullWidth select label='Loại' value={type} onChange={e => { setType(e.target.value); setPage(1) }} size='small'>
            ...
            <MenuItem value='ROTATING'>Proxy xoay</MenuItem>
      ...
      <Box sx={{ overflowX: 'auto', border: '1px solid #e2e8f0', borderRadius: 2 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          ...
          <tbody>
            {isLoading ? (
              <tr><td colSpan={7} style={{ textAlign: 'center', padding: 30, color: '#94a3b8' }}>Đang tải...</td></tr>
            ) : items.length === 0 ? (
              <tr><td colSpan={7} style={{ textAlign: 'center', padding: 30, color: '#94a3b8' }}>Chưa có proxy nào</td></tr>
                ...
                <tr key={item._id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={tdStyle}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                      <span style={{ fontFamily: 'monospace', fontSize: '11px', color: '#6366f1' }}>{item.key}</span>
                      <Tooltip title={copied === item.key ? 'Đã copy!' : 'Copy key'}>
                        <IconButton size='small' onClick={() => copyText(item.key, item.key)} sx={{ p: 0.25 }}>
                          {copied === item.key ? <Check size={12} color='#16a34a' /> : <Copy size={12} color='#94a3b8' />}
                        </IconButton>
                      </Tooltip>
                    </Box>
                  </td>
                  <td style={tdStyle}>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: '11px', color: item.type === 'ROTATING' ? '#f59e0b' : '#3b82f6' }}>
                      {item.type === 'ROTATING' ? <RefreshCw size={12} /> : <Shield size={12} />}
                  ...
                  <td style={tdStyle}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                      ...
                      {proxyText !== '—' && (
                        <Tooltip title={copied === `proxy-${item._id}` ? 'Đã copy!' : 'Copy proxy'}>
                          <IconButton size='small' onClick={() => copyText(proxyText, `proxy-${item._id}`)} sx={{ p: 0.25 }}>
                            {copied === `proxy-${item._id}` ? <Check size={12} color='#16a34a' /> : <Copy size={12} color='#94a3b8' />}
                          </IconButton>
                        </Tooltip>
                      )}
                    </Box>
                  ...
                  <td style={tdStyle}>
                    <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: '11px', fontWeight: 600, color: st.color, background: st.bg }}>
                      {st.label}
                    </span>
                  </td>
                  <td style={tdStyle}>
                  ...
                  <td style={tdStyle}>
                    {item.order_id ? (
                      <a href={`/${locale}/history-order`} style={{ fontSize: '11px', color: '#6366f1', display: 'inline-flex', alignItems: 'center', gap: 2 }}>
...
const thStyle: React.CSSProperties = { padding: '8px 10px', textAlign: 'left', fontSize: '12px', fontWeight: 600, color: '#64748b', borderBottom: '1px solid #e2e8f0' }
