...
#### 13.N+4 Reseller xem API Credentials — tích hợp vào Profile

**Vấn đề:** Reseller (role=2) không có chỗ nào trên FE để xem api_key/api_secret của mình. Chỉ admin mới xem được.

**Sửa:**
- `useRole.tsx`: thêm role `reseller` + permission `reseller.credentials` + helper `isReseller`/`useIsReseller`
- `useMyCredentials.ts`: hook gọi `GET /my-credentials`
- `CredentialsPanel.tsx`: tab mới trong Profile — hiển thị api_key (copy), api_secret (ẩn/hiện + copy), domain, company, markup, IPs, status
- `ProfilePage.tsx`: thêm tab "API Credentials" chỉ hiện cho role=reseller

**Files:** useRole.tsx, useMyCredentials.ts, CredentialsPanel.tsx, ProfilePage.tsx
